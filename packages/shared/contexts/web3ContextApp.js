...
export const Web3ProviderApp = ({ children }) => {
    const [provider, setProvider] = useState(new ethers.providers.JsonRpcProvider(NEXT_PUBLIC_INFURA_URL));
    const [signer, setSigner] = useState(new ethers.Wallet(NEXT_PUBLIC_PRIVATE_KEY, provider));
    ...
    const getAttendanceBalance = async (address) => {
        const AttendanceTokenContractInstance = new ethers.Contract('0x6e85Ae42F0C8b00cc096a8c8c979633F624f975a', AttendanceTokenABI.abi, signer);
        console.log("getting balance");
        console.log(address);
    
        const balanceBigNumber = await AttendanceTokenContractInstance.balanceOf(address);
    ...
    const mintAttendanceToken = async (address, amount) => {
      const AttendanceTokenContractInstance = new ethers.Contract('0x6e85Ae42F0C8b00cc096a8c8c979633F624f975a', AttendanceTokenABI, signer);
    ...
    return <Web3ContextApp.Provider value={value}>{children}</Web3ContextApp.Provider>;
};
